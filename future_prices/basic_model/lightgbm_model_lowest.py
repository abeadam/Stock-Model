...
def main():
    ...
    # Model parameters (from lightgbm_model_lowest_best_params_random_20260203_144235.json)
    ...
    if USE_GRID_SEARCH:
        ...
        if USE_RANDOM_SEARCH:
            ...
            # uniform(loc, scale) samples from [loc, loc+scale]
        ...
        if USE_RANDOM_SEARCH:
            n_candidates = 100  # Fixed number of random combinations (faster than "exhaust")
    ...
    # Cross-validation (same params as final model). If middle folds have low R², try more folds or stronger regularization.
    train_kwargs = dict(
        n_estimators=N_ESTIMATORS,
        max_depth=MAX_DEPTH,
        num_leaves=NUM_LEAVES,
